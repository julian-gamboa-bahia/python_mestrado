...
# A varíavel POLIGONO_TRUE_pontos_x, permite colocar as informações cálculadas no momento de passar os valores
	...
	temporal_criterio=response_data['criterio']	
	LEGENDA_TIPO_ANGULO_MATERIAL="".join([u'Tsai Hill  , com ângulo (Degree)  ', str(angulo),'   com el material  ',NOME])	
#GNU
...
def gerarGNU(request):	
	...
	response['Content-Disposition'] = 'attachment; filename="EnvelopesTsaiHill_%s.dat"' % (nomeTEMPO)
...
def matlab(request):
	...
	response['Content-Disposition'] = 'attachment; filename="EnvelopesTsaiHill_%s.txt"' % (nomeTEMPO)
