...
#Para larc03
	...
	temporal_criterio=response_data['criterio']	
	LEGENDA_TIPO_ANGULO_MATERIAL="".join([u'Larc03  , com ângulo (Degree)  ', str(angulo),'   com el material  ',NOME])	
#GNU
...
def calcula_if_grid(theta):
	...
	for i in range(0,len(POLIGONO_TRUE_pontos_x)):
		IF=larc03(POLIGONO_TRUE_pontos_x[i],POLIGONO_TRUE_pontos_y[i],tauXY,theta,SIGMA_T_1,SIGMA_T_2,SIGMA_C_1,SIGMA_C_2,TAU12)
...
#INICIO
	...
	if_inicial=larc03(corte_eixo_x,y,tauXY,angulo,SIGMA_T_1,SIGMA_T_2,SIGMA_C_1,SIGMA_C_2,TAU12)
#FIM
	...
	if_final=larc03(x,y,tauXY,angulo,SIGMA_T_1,SIGMA_T_2,SIGMA_C_1,SIGMA_C_2,TAU12)
...
# fim do while
		...
		x=novo
		y=x*coeficiente_angular+b
		if_inicial=larc03(x,y,tauXY,angulo,SIGMA_T_1,SIGMA_T_2,SIGMA_C_1,SIGMA_C_2,TAU12)
...
# Critério puro
#########################################################################################	
def larc03(sigma_x,sigma_y,tau_xy,theta,SIGMA_T_1,SIGMA_T_2,SIGMA_C_1,SIGMA_C_2,TAU12):
...
########################################################################################################################################################################	
	...
	IFs=[]
	IFs.append(if_larc_03_tracao_fibra)
	IFs.append(if_larc_03_tracao_matriz)
	IFs.append(if_larc_03_compressao_matriz_larc0301)
	IFs.append(if_larc_03_compressao_matriz_larc03_06)

	IFs.append(if_larc_03_compressao_fibra_larc03_04)
	IFs.append(if_larc_03_compressao_fibra_larc03_05)
	return IFs
...
# Auxilio
#########################################################################################		
def calcular_Fi(sigma_1,sigma_2,tau_12,theta,SIGMA_T_1,SIGMA_T_2,SIGMA_C_1,SIGMA_C_2,TAU12):
	alpha_0=53.2*(Math.pi/180)
	#S_L_is=Math.sqrt(2)*TAU12
	eta_L=(TAU12*Math.cos(alpha_0*2))/(SIGMA_C_2*Math.pow(Math.cos(alpha_0),2))
...
#Cuidado				
				while((o<=0.0) and (proximoTOP>=0.0)):
					...
					o=ccw(stack_x[proximoTOP],stack_x[TOP],pontos_ordenados_x[i],stack_y[proximoTOP],stack_y[TOP],pontos_ordenados_y[i])					 
					if(o>0.0):
						print("Testando dentro do While %f Será inserido o %f" % (o,pontos_ordenados_x[i]))
				stack_x.append(pontos_ordenados_x[i])
				stack_y.append(pontos_ordenados_y[i])
		print "##Estudo de O"    			
		print stack_x    			
# end FOR
	...
	print len(POLIGONO_TRUE_pontos_x)
	print POLIGONO_TRUE_pontos_y
	print len(POLIGONO_TRUE_pontos_y)	
...
def ccw(p1_x, p2_x, p3_x,p1_y, p2_y, p3_y):
	...
	print ("Testando com: %f %f %f" % (p1_x, p2_x, p3_x))
	print ("Testando com: %f" % ((p2_x - p1_x)*(p3_y - p1_y) - (p2_y - p1_y)*(p3_x - p1_x)))
...
# de entrada 
#########################################################################################		
def coletar_alpha(request,alpha_entrada):
	global alpha
	alpha=float(alpha_entrada)
	return HttpResponse("alpha_entrada  "+alpha_entrada)
...
def coletar_TAU23(request,TAU23_entrada):
	global TAU23
	TAU23=float(TAU23_entrada)	
	return HttpResponse("TAU23  "+TAU23_entrada)	
#########################################################################################
# Nov05
# SUPORTE para calculo_envelope
# CAIXAS
# de entrada 
#########################################################################################		
def coletar_Y_T_is(request,Y_T_is_entrada):
	global Y_T_is
	Y_T_is=float(Y_T_is_entrada)	
	return HttpResponse("Y_T_is  "+Y_T_is_entrada)	
#########################################################################################
# Nov05
# SUPORTE para calculo_envelope
# CAIXAS
# de entrada 
#########################################################################################		
def coletar_S_L_is(request,S_L_is_entrada):
	global S_L_is
	S_L_is=float(S_L_is_entrada)	
	print "######################## 		Nov05"
	print "S_L_is"
	print S_L_is
	return HttpResponse("S_L_is  "+S_L_is_entrada)	
...
def gerarGNU(request):	
	...
	response['Content-Disposition'] = 'attachment; filename="EnvelopesLarc03_%s.dat"' % (nomeTEMPO)
...
def matlab(request):
	...
	response['Content-Disposition'] = 'attachment; filename="EnvelopesLarc03_%s.txt"' % (nomeTEMPO)
